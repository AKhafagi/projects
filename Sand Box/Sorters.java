...
public class Sorters<E> {
	 ...
	 * then merges those arrays with their neighbors to form partitions of size 2 , 
	 * then size 4 and so on until the array is in sorted order
	 * In place: No, as it requires more memory as the input size increases<br>
	 * Stable: Yes
	 * Complexity
	 * Best: O(nlog(n))
	 * Worst: O(nlog(n))
	 ...
	 * @param order  the Comparator object to consult when ranking the elements
	 * @return the sorted array 
	 */
	...
	/**
	 * merges each array with its neighbor to build the array back in sorted order
	 * @param low the smallest index
	 * @param hi the largest index
	 * @param upperBound the upperbound of the array
	...
	/**
	 * Quick sort recursively partitions the array into smaller pieces based on a pivot
	 * the elements are arranged so that everything on the left of the pivot is smaller than the pivot
	 * and everything to the right greater than the pivot. <br>
	 * In place: Yes.
	 * Stable: No.
	 * Complexity
	 * Best: O(nlog(n))
	 ...
	 * Best: O(nlog(n))
	 * Average: O(nlog(n))
	 * Worst: O(n^2) the worst case is when the data is in order, or almost sorted order.
	 * 
	 * @param toSort the array to be sorted.
	 * @param order the Comparator object to consult when ranking the elements.
	 * @return the sorted array.
	...
	/**
	 * @param left the left most index of the partition	
	 * @param right the right most index of the partition
	 * @param pivot the pivot where everything to the left is smaller and everything to the right is greater
	 * @param array the array to be sorted 
	 * @param order the Comparator object to consult when ranking the elements
	 ...
	 * @param array the array to swap the elements from
	 */
	...
	/**
	 * Shell sort works by repeatedly sort small sections of the array by using insertion sort.
	 * these small sections span the entire array and are compromised of every nth element. then n decreases until n=1.<br>
	 * In place: Yes<br>
	 * Stable: No
	 * Complexity
	 * Best: O(n^3/2)
